...
if ! sbx ls >/dev/null 2>&1; then
    die "sbx не авторизован или демон недоступен. Выполните: sbx login (запуск демона: sbx daemon start -d)"
fi

# Разовая network policy: LLM-порт хоста достижим как host.docker.internal:<port>.
if ! sbx policy ls 2>/dev/null | grep -q "localhost:${LLM_PORT}"; then
    say "Добавляю network policy localhost:${LLM_PORT} (разовая операция)"
    sbx policy allow network "localhost:${LLM_PORT}"
fi
...
if sbx ls 2>/dev/null | awk '{print $1}' | grep -qx "${SANDBOX_NAME}"; then
    ...
    sbx create --name "${SANDBOX_NAME}" shell "${WORKSPACE}"
fi
